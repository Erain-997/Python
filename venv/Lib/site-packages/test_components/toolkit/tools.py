...
class SmallTools(GetTestParams, AllureMethod):
    ...
    def compare_dict_result(cls, original, reference, ignore=None, retain=None, result=True, assert_key_list=None,
                            need_sort=True, check_contain=False):
        """
        字典比对（默认相同层级）
        ...
        :param check_contain: 不同层级（判断original与reference之间，内容多的是否包含内容少的）
        """
        ...
        if check_contain:
            ...
            return cls.compare_contain_dict(real_origin, real_reference, assert_key_list, result, need_sort)
        if ignore is not None:
            for i in ignore:
                if i in assert_key_list:
                    assert_key_list.remove(i)
        return cls.compare_dict(original, reference, assert_key_list, result, need_sort)
    ...
    @classmethod
    def compare_dict(cls, original, reference, key_list, result, need_sort):
        """
        字典比较
        :param original:
        :param reference:
            ...
            for key in key_list:
                ...
                if key in original_keys and key in reference_keys:
                    if isinstance(cls.get_dict_value(original, key), dict):
                        # 如果是字典类型
                        if result:
                            cls.compare_dict(original[key], reference[key], cls.get_dict_pk_key(original[key], []),
                                             result,
                                             need_sort)
                        else:
                            return cls.compare_dict(original[key], reference[key],
                                                    cls.get_dict_pk_key(original[key], []),
                                                    result, need_sort)
                                ...
                                if isinstance(original[key][0], dict):
                                    ...
                                    for sorted_key in sorted_keys:
                                        ...
                                        if not isinstance(original[key][0][sorted_key], dict) and not isinstance(
                                            ...
                                            break
                                    sorted_origin = sorted(original[key], key=operator.itemgetter(str_sorted_key))
                                    sorted_reference = sorted(reference[key], key=operator.itemgetter(str_sorted_key))
                                    if result:
                                        for son_index in sorted_origin:
                                            # 断言列表里origin存在于reference中
                                            assert son_index in sorted_reference, "%s 预期：%s存在于%s中" % (
                                                key, son_index, sorted_reference)
                                    else:
                                    ...
                                    if result:
                                        assert dif_set == [] and len(original[key]) == len(reference[key]), \
                                            "%s 预期：%s，实际：%s" % (key, reference[key], original[key])
                                    else:
                                        return dif_set == [] and len(original[key]) == len(reference[key])
                            else:
                                assert len(original[key]) == 0 and len(reference[key]) == 0, "%s 预期：%s，实际：%s" % (
                                    key, reference[key], original[key])
                        else:
                            if result:
                                assert original[key] == reference[key], "%s 预期：%s，实际：%s" % (
                                    key, reference[key], original[key])
    ...
    @classmethod
    def compare_contain_dict(cls, original, reference, key_list, result, need_sort):
        """
        字典比较（可以是不同层级）
        :param original:
        :param reference:
                    ...
                    if isinstance(origin_value, dict):
                        ...
                        if result:
                            ...
                            return cls.compare_dict(origin_value, reference_value,
                                                    cls.get_dict_pk_key(origin_value, []), result, need_sort)
